---
title: "Gemini 3.7 Flash — Model Overview"
type: model-note
year: 2026
...
# Gemini 3.7 Flash — Model Overview

> [!tldr]
> 当前 Flash 正代，强化 coding、agentic workflow 与可靠多步执行。 **我的判断：3.7 Flash 应作为当前默认可见叶子；向右滑动才回溯 3.6、3.5 与更早模型。**

![Gemini 3.7 Flash 证据地图](src/assets/evidence-map.svg)
...
Gemini 3.7 Flash 不是孤立 SKU，而是这条主线能力重心的一次迁移。上一代积累的通用语言能力仍是底座，但这一节点把评价重点移到：**当前 Flash 正代，强化 coding、agentic workflow 与可靠多步执行。** 因此阅读它时不能只看一张 benchmark 表，而要看模型进入真实工作流后，规划、上下文管理、工具调用和失败恢复如何协同。

## 三个必须记住的事实

1. **1,048,576 input tokens**：这是区分前后代的第一条硬证据。
2. **65,536 max output tokens**：它决定模型更适合怎样的上下文与工作负载。
3. **text / image / video / audio / PDF input**：它说明 family 的产品接口或能力边界如何变化。

## 技术变化怎么理解

### 1. 基础能力层

官方材料可以确认的核心转向是：当前 Flash 正代，强化 coding、agentic workflow 与可靠多步执行。 这比“模型更聪明”更精确，因为它给出了比较维度；静态知识问答、长上下文利用、推理预算、多模态输入与工具执行不能混成一个总分。
...
- [API Model Page](https://ai.google.dev/gemini-api/docs/models/gemini-3.7-flash)
...
- [[gemini_3_7_flash_poster_zh|中文 Poster]]
