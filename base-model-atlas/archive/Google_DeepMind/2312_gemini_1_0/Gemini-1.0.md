---
title: "Gemini 1.0 — Model Overview"
type: model-note
year: 2023
...
# Gemini 1.0 — Model Overview

> [!tldr]
> Ultra / Pro / Nano 把原生多模态与设备—云端分层放进同一 family。 **我的判断：Gemini 1.0 的核心不是后接视觉模块，而是把多模态作为基础训练目标。**

![Gemini 1.0 证据地图](src/assets/evidence-map.svg)
...
Gemini 1.0 不是孤立 SKU，而是这条主线能力重心的一次迁移。上一代积累的通用语言能力仍是底座，但这一节点把评价重点移到：**Ultra / Pro / Nano 把原生多模态与设备—云端分层放进同一 family。** 因此阅读它时不能只看一张 benchmark 表，而要看模型进入真实工作流后，规划、上下文管理、工具调用和失败恢复如何协同。

## 三个必须记住的事实

1. **Ultra / Pro / Nano**：这是区分前后代的第一条硬证据。
2. **原生 text / image / audio / video**：它决定模型更适合怎样的上下文与工作负载。
3. **从数据到模型的多模态联合训练**：它说明 family 的产品接口或能力边界如何变化。

## 技术变化怎么理解

### 1. 基础能力层

官方材料可以确认的核心转向是：Ultra / Pro / Nano 把原生多模态与设备—云端分层放进同一 family。 这比“模型更聪明”更精确，因为它给出了比较维度；静态知识问答、长上下文利用、推理预算、多模态输入与工具执行不能混成一个总分。
...
- [Tech Blog](https://deepmind.google/technologies/gemini/)
- [Tech Report / System Report](https://arxiv.org/pdf/2312.11805)
...
- [[gemini_1_0_poster_zh|中文 Poster]]
