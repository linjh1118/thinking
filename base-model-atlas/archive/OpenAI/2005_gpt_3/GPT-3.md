...
> [!tldr]
> few-shot / in-context learning 成为通用模型的主要交互范式。 **我的判断：GPT-3 的真正代际意义是把样例放进上下文，而不是为每个任务更新权重。**

![GPT-3 官方发布图](src/assets/official-hero.png)

![GPT-3 证据地图](src/assets/evidence-map.svg)
...
GPT-3 不是孤立 SKU，而是这条主线能力重心的一次迁移。上一代积累的通用语言能力仍是底座，但这一节点把评价重点移到：**few-shot / in-context learning 成为通用模型的主要交互范式。** 因此阅读它时不能只看一张 benchmark 表，而要看模型进入真实工作流后，规划、上下文管理、工具调用和失败恢复如何协同。

## 三个必须记住的事实

1. **175B 参数**：这是区分前后代的第一条硬证据。
2. **2,048 token 上下文**：它决定模型更适合怎样的上下文与工作负载。
3. **zero/one/few-shot 统一评测**：它说明 family 的产品接口或能力边界如何变化。

## 技术变化怎么理解

### 1. 基础能力层

官方材料可以确认的核心转向是：few-shot / in-context learning 成为通用模型的主要交互范式。 这比“模型更聪明”更精确，因为它给出了比较维度；静态知识问答、长上下文利用、推理预算、多模态输入与工具执行不能混成一个总分。
...
> [!insight]
> GPT-3 的真正代际意义是把样例放进上下文，而不是为每个任务更新权重。
...
> [!warning]
> 技术报告披露训练与评测；部署、安全与后续 API 行为不能从论文直接外推。

因此这份笔记的目标是达到**一手资料能支持的最高标准**，而不是用通用套话伪造参数、训练数据、架构和消融。若后续出现新的 Tech Report / System Card，应优先补进 `src/` 并据此重写技术细节。

## 一手资料

- [Tech Report](https://arxiv.org/abs/2005.14165)
- [Tech Report / System Report](https://arxiv.org/pdf/2005.14165)
