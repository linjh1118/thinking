...
# GPT-4.5 — Model Overview

> [!tldr]
> 继续扩大无监督学习，强化世界知识、模式识别与自然交流。 **我的判断：GPT-4.5 是 OpenAI 对“更大预训练”和“更强推理”两条扩展轴的显式区分。**

![GPT-4.5 官方发布图](src/assets/official-hero.png)

![GPT-4.5 证据地图](src/assets/evidence-map.svg)
...
GPT-4.5 不是孤立 SKU，而是这条主线能力重心的一次迁移。上一代积累的通用语言能力仍是底座，但这一节点把评价重点移到：**继续扩大无监督学习，强化世界知识、模式识别与自然交流。** 因此阅读它时不能只看一张 benchmark 表，而要看模型进入真实工作流后，规划、上下文管理、工具调用和失败恢复如何协同。

## 三个必须记住的事实

1. **research preview**：这是区分前后代的第一条硬证据。
2. **强调预训练 scaling 而非显式 reasoning**：它决定模型更适合怎样的上下文与工作负载。
3. **自然度与低幻觉是主要产品目标**：它说明 family 的产品接口或能力边界如何变化。

## 技术变化怎么理解

### 1. 基础能力层

官方材料可以确认的核心转向是：继续扩大无监督学习，强化世界知识、模式识别与自然交流。 这比“模型更聪明”更精确，因为它给出了比较维度；静态知识问答、长上下文利用、推理预算、多模态输入与工具执行不能混成一个总分。
...
- [Tech Blog](https://openai.com/index/introducing-gpt-4-5/)
