...
> [!tldr]
> 指令对齐、RLHF 与对话产品把通用基模变成可规模化的人机接口。 **我的判断：这代最重要的是 post-training 与产品反馈闭环，而不是公开的架构变化。**

![GPT-3.5 证据地图](src/assets/evidence-map.svg)
...
1. **对话式多轮交互**：这是区分前后代的第一条硬证据。
2. **人类反馈驱动的行为对齐**：它决定模型更适合怎样的上下文与工作负载。
3. **从基础模型转向持续迭代服务**：它说明 family 的产品接口或能力边界如何变化。
...
> [!insight]
> 这代最重要的是 post-training 与产品反馈闭环，而不是公开的架构变化。
...
> [!warning]
> 缺少独立 GPT-3.5 技术报告；笔记只能达到官方产品页 + RLHF 路线证据标准。

因此这份笔记的目标是达到**一手资料能支持的最高标准**，而不是用通用套话伪造参数、训练数据、架构和消融。若后续出现新的 Tech Report / System Card，应优先补进 `src/` 并据此重写技术细节。

## 一手资料

- [Product Blog](https://openai.com/index/chatgpt/)
- [Tech Report / System Report](https://arxiv.org/pdf/2203.02155)
