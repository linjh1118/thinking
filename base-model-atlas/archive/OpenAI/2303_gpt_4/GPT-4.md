...
> [!tldr]
> 高可靠复杂任务与图像输入进入同一闭源旗舰。 **我的判断：GPT-4 确立了能力报告与安全评估并行发布的闭源旗舰范式。**

![GPT-4 证据地图](src/assets/evidence-map.svg)
...
GPT-4 不是孤立 SKU，而是这条主线能力重心的一次迁移。上一代积累的通用语言能力仍是底座，但这一节点把评价重点移到：**高可靠复杂任务与图像输入进入同一闭源旗舰。** 因此阅读它时不能只看一张 benchmark 表，而要看模型进入真实工作流后，规划、上下文管理、工具调用和失败恢复如何协同。

## 三个必须记住的事实

1. **文本 + 图像输入**：这是区分前后代的第一条硬证据。
2. **专业考试能力显著跃迁**：它决定模型更适合怎样的上下文与工作负载。
3. **以 system card 管理能力与风险**：它说明 family 的产品接口或能力边界如何变化。

## 技术变化怎么理解

### 1. 基础能力层

官方材料可以确认的核心转向是：高可靠复杂任务与图像输入进入同一闭源旗舰。 这比“模型更聪明”更精确，因为它给出了比较维度；静态知识问答、长上下文利用、推理预算、多模态输入与工具执行不能混成一个总分。
...
> [!warning]
> 官方刻意未披露参数、训练数据与算力；任何架构数字都不应写成事实。

因此这份笔记的目标是达到**一手资料能支持的最高标准**，而不是用通用套话伪造参数、训练数据、架构和消融。若后续出现新的 Tech Report / System Card，应优先补进 `src/` 并据此重写技术细节。

## 一手资料

- [Tech Blog](https://openai.com/research/gpt-4)
- [Tech Report / System Report](https://cdn.openai.com/papers/gpt-4.pdf)
