---
title: "Claude Sonnet 5 — Model Overview"
type: model-note
year: 2026
...
# Claude Sonnet 5 — Model Overview

> [!tldr]
> 把新一代高阶 agent 能力下放到 Sonnet 的成本与延迟层。 **我的判断：Sonnet 5 证明 Claude 主线是能力下放 + 可靠执行，而不是只刷新顶配。**

![Claude Sonnet 5 官方发布图](src/assets/official-hero.jpg)

![Claude Sonnet 5 证据地图](src/assets/evidence-map.svg)
...
Claude Sonnet 5 不是孤立 SKU，而是这条主线能力重心的一次迁移。上一代积累的通用语言能力仍是底座，但这一节点把评价重点移到：**把新一代高阶 agent 能力下放到 Sonnet 的成本与延迟层。** 因此阅读它时不能只看一张 benchmark 表，而要看模型进入真实工作流后，规划、上下文管理、工具调用和失败恢复如何协同。

## 三个必须记住的事实

1. **Sonnet 新正代**：这是区分前后代的第一条硬证据。
2. **长时程 agents**：它决定模型更适合怎样的上下文与工作负载。
3. **官方回溯 3.5 / 3.6 / 3.7 谱系**：它说明 family 的产品接口或能力边界如何变化。

## 技术变化怎么理解

### 1. 基础能力层

官方材料可以确认的核心转向是：把新一代高阶 agent 能力下放到 Sonnet 的成本与延迟层。 这比“模型更聪明”更精确，因为它给出了比较维度；静态知识问答、长上下文利用、推理预算、多模态输入与工具执行不能混成一个总分。
...
- [Tech Blog](https://www.anthropic.com/news/claude-sonnet-5)
...
- [[claude_sonnet_5_poster_zh|中文 Poster]]
