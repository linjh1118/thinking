---
title: "Gemini 2.0 — Model Overview"
type: model-note
year: 2024
...
# Gemini 2.0 — Model Overview

> [!tldr]
> 原生工具、实时交互和多模态输出把 Gemini 推向 agentic era。 **我的判断：Gemini 2.0 的分水岭是模型开始主动调用搜索、代码与界面，而非只消费上下文。**

![Gemini 2.0 证据地图](src/assets/evidence-map.svg)
...
Gemini 2.0 不是孤立 SKU，而是这条主线能力重心的一次迁移。上一代积累的通用语言能力仍是底座，但这一节点把评价重点移到：**原生工具、实时交互和多模态输出把 Gemini 推向 agentic era。** 因此阅读它时不能只看一张 benchmark 表，而要看模型进入真实工作流后，规划、上下文管理、工具调用和失败恢复如何协同。

## 三个必须记住的事实

1. **native tool use**：这是区分前后代的第一条硬证据。
2. **实时多模态交互**：它决定模型更适合怎样的上下文与工作负载。
3. **面向 agent prototypes**：它说明 family 的产品接口或能力边界如何变化。

## 技术变化怎么理解

### 1. 基础能力层

官方材料可以确认的核心转向是：原生工具、实时交互和多模态输出把 Gemini 推向 agentic era。 这比“模型更聪明”更精确，因为它给出了比较维度；静态知识问答、长上下文利用、推理预算、多模态输入与工具执行不能混成一个总分。
...
> [!insight]
> Gemini 2.0 的分水岭是模型开始主动调用搜索、代码与界面，而非只消费上下文。
...
> [!warning]
> 发布页能支持产品架构判断；完整训练与后训练 recipe 未公开。

因此这份笔记的目标是达到**一手资料能支持的最高标准**，而不是用通用套话伪造参数、训练数据、架构和消融。若后续出现新的 Tech Report / System Card，应优先补进 `src/` 并据此重写技术细节。

## 一手资料

- [Tech Blog](https://blog.google/technology/google-deepmind/google-gemini-ai-update-december-2024/)
...
- [[gemini_2_0_poster_zh|中文 Poster]]
