---
title: "Gemini 3 Pro — Model Overview"
type: model-note
year: 2025
...
# Gemini 3 Pro — Model Overview

> [!tldr]
> 多模态 reasoning、coding 与 agent 工具链进入 Gemini 3 正代。 **我的判断：Gemini 3 Pro 应按完整 Google 工具生态内的任务完成度评估。**

![Gemini 3 Pro 证据地图](src/assets/evidence-map.svg)
...
| 发布时间 | 2025-11 |
| Atlas 归类 | 主干正代 |
| 主要证据 | Official Model Page |
| 证据充分度 | 见“资料边界”，不把产品页补写成论文 |

Gemini 3 Pro 不是孤立 SKU，而是这条主线能力重心的一次迁移。上一代积累的通用语言能力仍是底座，但这一节点把评价重点移到：**多模态 reasoning、coding 与 agent 工具链进入 Gemini 3 正代。** 因此阅读它时不能只看一张 benchmark 表，而要看模型进入真实工作流后，规划、上下文管理、工具调用和失败恢复如何协同。

## 三个必须记住的事实

1. **Pro 能力层**：这是区分前后代的第一条硬证据。
2. **多模态 reasoning**：它决定模型更适合怎样的上下文与工作负载。
3. **agent / tool ecosystem**：它说明 family 的产品接口或能力边界如何变化。

## 技术变化怎么理解

### 1. 基础能力层

官方材料可以确认的核心转向是：多模态 reasoning、coding 与 agent 工具链进入 Gemini 3 正代。 这比“模型更聪明”更精确，因为它给出了比较维度；静态知识问答、长上下文利用、推理预算、多模态输入与工具执行不能混成一个总分。
...
> [!insight]
> Gemini 3 Pro 应按完整 Google 工具生态内的任务完成度评估。
...
- [[gemini_3_pro_poster_zh|中文 Poster]]
