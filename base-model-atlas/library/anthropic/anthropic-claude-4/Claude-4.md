...
> [!tldr]
> Opus / Sonnet 进入长时程 coding agents 与持续工具使用阶段。 **我的判断：Claude 4 的评价单位从单轮答案转向数小时软件工程轨迹。**

![Claude 4 官方发布图](src/assets/official-hero.png)

![Claude 4 证据地图](src/assets/evidence-map.svg)
...
Claude 4 不是孤立 SKU，而是这条主线能力重心的一次迁移。上一代积累的通用语言能力仍是底座，但这一节点把评价重点移到：**Opus / Sonnet 进入长时程 coding agents 与持续工具使用阶段。** 因此阅读它时不能只看一张 benchmark 表，而要看模型进入真实工作流后，规划、上下文管理、工具调用和失败恢复如何协同。

## 三个必须记住的事实

1. **Opus 4 + Sonnet 4**：这是区分前后代的第一条硬证据。
2. **长时程 agentic coding**：它决定模型更适合怎样的上下文与工作负载。
3. **extended thinking + tool use**：它说明 family 的产品接口或能力边界如何变化。

## 技术变化怎么理解

### 1. 基础能力层

官方材料可以确认的核心转向是：Opus / Sonnet 进入长时程 coding agents 与持续工具使用阶段。 这比“模型更聪明”更精确，因为它给出了比较维度；静态知识问答、长上下文利用、推理预算、多模态输入与工具执行不能混成一个总分。
...
> [!insight]
> Claude 4 的评价单位从单轮答案转向数小时软件工程轨迹。
