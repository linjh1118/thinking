...
# Claude Opus 4.6 — Model Overview

> [!tldr]
> 以更长上下文与更强规划推进复杂专业 agents。 **我的判断：Opus 4.6 应被视为复杂工作流的规划器，而不是只做一次性答案生成。**

![Claude Opus 4.6 官方发布图](src/assets/official-hero.png)

![Claude Opus 4.6 证据地图](src/assets/evidence-map.svg)
...
Claude Opus 4.6 不是孤立 SKU，而是这条主线能力重心的一次迁移。上一代积累的通用语言能力仍是底座，但这一节点把评价重点移到：**以更长上下文与更强规划推进复杂专业 agents。** 因此阅读它时不能只看一张 benchmark 表，而要看模型进入真实工作流后，规划、上下文管理、工具调用和失败恢复如何协同。

## 三个必须记住的事实

1. **Opus 最高能力层**：这是区分前后代的第一条硬证据。
2. **1M context 级工作负载**：它决定模型更适合怎样的上下文与工作负载。
3. **agent planning**：它说明 family 的产品接口或能力边界如何变化。

## 技术变化怎么理解

### 1. 基础能力层

官方材料可以确认的核心转向是：以更长上下文与更强规划推进复杂专业 agents。 这比“模型更聪明”更精确，因为它给出了比较维度；静态知识问答、长上下文利用、推理预算、多模态输入与工具执行不能混成一个总分。
...
> [!insight]
> Opus 4.6 应被视为复杂工作流的规划器，而不是只做一次性答案生成。
...
- [Tech Blog](https://www.anthropic.com/news/claude-opus-4-6)
...
- [[claude_opus_4_6_poster_zh|中文 Poster]]
